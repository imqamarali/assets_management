    ...
    <div class="row">
        <div class="d-flex" id="scrollspyEcommerce">
            ...
            <div class="col">
                ...
                <p class="mb-0">Route Wise KM & Status</p>
            </div>
...
//echo 'SELECT * FROM public."n_asset" where "Route_id"='.$item['id'].' order by id ASC';exit;
								...
								$max=Yii::$app->db->createCommand('SELECT n_asset.name,a_tehsil.name as tname FROM public."n_asset" 
								left join a_tehsil on (a_tehsil.id=techsil_id)
								where "Route_id"='.$item['id'].' order by n_asset.id ASC')->queryOne();
								$min=Yii::$app->db->createCommand('SELECT n_asset.name,a_tehsil.name as tname FROM public."n_asset" 
								left join a_tehsil on (a_tehsil.id=techsil_id)
								where "Route_id"='.$item['id'].' order by n_asset.id DESC')->queryOne();
								// $min['name'];exit;
                            ?>
                                <tr>
