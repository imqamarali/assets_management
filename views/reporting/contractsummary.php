<div class="mx-n4 px-4 mx-lg-n6 px-lg-6 bg-white">
    <div class="row">
        <div class="d-flex" id="scrollspyEcommerce">
            ...
            <div class="col">
                <h3 class="mb-0 text-primary position-relative fw-bold"><span class="bg-soft pe-2">Contracts Summary & Schedule </span>
                    <span
                        class="border border-primary-200 position-absolute top-50 translate-middle-y w-100 start-0 z-index--1"></span>
                </h3>
                <p class="mb-0">Contracts Summary & Schedule for Next 3 month  </p>
            </div>

           
        </div>
         <form method="POST" id="ajaxform" action="index.php?r=reporting/contractsummary">
...
<div class="col-12 col-md-12" >
       <label for="organizerSingle2">Contract No.</label>
		<input type="text" name="contractno" class="form-select form-select-sm"/>
</div>
<div class="col-12 col-md-12" >
...
<div class="col-12 col-md-12" >
       <label for="organizerSingle2">Province</label>
		<select id="province" class="form-select form-select-sm" aria-label=".form-select-sm example" onclick="district()" name="province">
<option value="">Please Region</option>

<?php 
$amp_pro = Yii::$app->db->createCommand('SELECT * FROM public."a_province"')->queryAll();
        foreach ($amp_pro as $value) {
            //$selected = ($value == $submittedValue) ? 'selected' : '';
            echo '<option value="' . $value['ID'] . '">' . $value['name'] . '</option>';
        }
?>
...
<div class="col-12 col-md-12" >
       <label for="organizerSingle2">District</label>
		<select id="district" class="form-select form-select-sm" aria-label=".form-select-sm example"  name="district">
<option value="">Please District</option>

</select>
</div>
<div class="col-12 col-md-12" >
       <label for="organizerSingle2">Status</label>
...
<option value="0">Under Approval</option>
<option value="1">Inprogress</option>
<option value="2">Completed</option>

</select>
</div>
		<input type="submit"  name="apply_search" class="btn btn-outline-primary" style="margin-top: 24px;" value="Search"  >
	   </div>
                ...
                <div class="table-responsive">
                    <table id="" class="table table-striped table-hover table-sm fs--1 mb-0 simlee">
                        <thead>
                            ...
                            <th class="sort border-top" style="width:250px !important">Contractor Name</th>
							
							<th class="sort border-top">Route</th>
                            <th class="sort border-top">Bid Cost</th>
                            <th class="sort border-top">Paid</th>
							
                            <th class="sort border-top">Remaining</th>
							<th class="sort border-top">Progress</th>
							<th class="sort border-top">Financial Progress</th>
							<th class="sort border-top">Demand Of Fund </th>							
                            </tr>
                        </thead>
                        <tbody class="list">
                            <?php $index = 1;
                            foreach ($assets_list as $item):
								...
								//$count=Yii::$app->db->createCommand('SELECT count(*) FROM public."n_asset" where "Route_id"='.$item['id'].'')->queryOne();
								//$MAX=Yii::$app->db->createCommand('SELECT * FROM public."n_asset" where "Route_id"='.$item['id'].' order by id ASC')->queryOne();
								//$MIN=Yii::$app->db->createCommand('SELECT * FROM public."n_asset" where "Route_id"='.$item['id'].' order by id DESC')->queryOne();
								//print_r( $count);exit;
								 $paidamount = Yii::$app->db->createCommand('SELECT sum(amount) as amount FROM public."m_contract_payments" where contract_id='.$item['id'])->queryOne(); ?>   
                            
							<?php
							$statuss=1;
							if($paidamount['amount']==0){$paidamount['amount']=1;}
							if($item['bid_cost']==0){$item['bid_cost']=1;}
							$progress=round(($paidamount['amount']/$item['bid_cost'])*100)+rand(1,5);
							if($progress>=100){$progress=100;$statuss=2;}
							$obj = [
                            'progress'=>$progress,
                            'status' => $statuss
                        ];
							//Yii::$app->db->createCommand()->update('m_contract', $obj, ['id' => $item['id']])->execute();
							?>
                                <tr>
                                    <td class="center"><?= $index++ ?></td>
                                    
                                <td ><b><?= $item['finance_ref_code'] ?></b></td>
                                <td><?= $item['contract_no'] ?></td>
                                
								<td><?= $item['route_name'] ?></td>
                                <td style="color:blue;"><?= number_format($item['bid_cost']) ?></td>
								<td style="color:blue;"><?= number_format($paidamount['amount']) ?></td>
								<td style="color:blue;"><?= number_format($item['bid_cost']-$paidamount['amount']) ?></td>
                                <td style="color:red;"><?= number_format($item['progress']).' %' ?></td>
                                <td style="color:green;"><?= number_format(($paidamount['amount']/$item['bid_cost'])*100,2).' %' ?></td>
                                <td style="color:red;"><?= number_format(000) ?></td>
...
function select_region(id){
	...
	function district(id){
        $.ajax({
            type: "POST",
            url: "http://<?php echo $address; ?>/index.php?r=site/district&id=" + document.getElementById("province").value,
            contenetType: "json",
